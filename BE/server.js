...
app.post('/api/auth/google', async (req, res) => {
  ...
  try {
    ...
    // ===============================
    // ✅ 1) ดึง role และ status จาก DATABASE
    // ===============================
    ...
    if (rows.length > 0) {
      ...
      // ✅ ตรวจสอบตาม role
      if (role === 1) {
        // โรล 1 (Admin) - เข้าได้เลย
        // ไม่ต้องตรวจสอบอะไร
      } else if (role === 2) {
        // โรล 2 - ต้องตรวจสอบ status
        if (status !== 1) {
          return res.status(403).json({
            success: false,
      ...
      // role 3 หรืออื่นๆ เข้าได้ปกติ

    } else {
      // ❗ไม่พบ user ในระบบ
      // ให้ล๊อกอินได้เลยโดยไม่ต้อง insert และกำหนด role = 3
      userId = null; // ไม่มี user_id จาก database
      role = 3;
    }
