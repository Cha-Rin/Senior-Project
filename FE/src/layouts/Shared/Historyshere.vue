...
const { history, startDate, endDate, showCalendar, currentMonth, currentYear, toggleCalendar, calendarDays, currentMonthName, filteredHistory, resetDate, formatDateDisplay, formatDateTime, changeMonth } = useHistory()
...
onMounted(async () => {
  ...
  const role = localStorage.getItem('role') // 'student' | 'secretary' | 'teacher'
  if(!userId) return
