...
onMounted(async () => {
  ...
  try {
    ...
    const res = await fetch('http://localhost:3000/student/appointment-topics', {
      headers: {
...
async function loadAppointments() {
  try {
    ...
    const res = await fetch('http://localhost:3000/student/appointments/for-feedback?approved_set=1', {
      headers: {
        'Authorization': `Bearer ${token}`,
        'Content-Type': 'application/json'
      }
